...
function completedSteps(draft: CandidateDraft) {
  const steps = [
    ...
    draft.introZoomScheduled && Boolean(draft.introZoomDate),
    ...
    draft.thirdCertificationScheduled && Boolean(draft.thirdCertificationDate),
    Boolean(draft.thirdCertificationPreparationZoomDate),
    ...
    <GlassCard className={isRejected ? 'opacity-55 grayscale' : ''}>
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-2 text-sm text-[#a89b8f]">Заполнено этапов: {done} из 11</p>
        </div>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <div className="rounded-xl border border-[#4d3f3a] bg-[#1c1820] p-4">
          ...
          <BooleanField checked={draft.introZoomScheduled} disabled={isRejected} label="Зум назначен" onChange={(value) => setDraft((current) => ({ ...current, introZoomScheduled: value, introZoomDate: value ? current.introZoomDate : null }))} />
          <input
            type="date"
        ...
        <div className="rounded-xl border border-[#4d3f3a] bg-[#1c1820] p-4 xl:col-span-2">
          <label className="mb-2 block text-sm font-semibold text-[#d8c7b7]">5. Вторая аттестация</label>
        ...
        <BooleanPanel title="6. Посещение тренировок после 2 аттестации" checked={draft.trainingsVisitedAfterSecondCertification} disabled={isRejected} label="Тренировки посещены" onChange={(value) => setField('trainingsVisitedAfterSecondCertification', value)} />
        <BooleanPanel title="7. Фото и видео для клиентов и CRM" checked={draft.mediaCollected} disabled={isRejected} label="Материалы собраны" onChange={(value) => setField('mediaCollected', value)} />

        <div className="rounded-xl border border-[#4d3f3a] bg-[#1c1820] p-4">
          <label className="mb-2 block text-sm font-semibold text-[#d8c7b7]">8. Третья аттестация</label>
          <BooleanField checked={draft.thirdCertificationScheduled} disabled={isRejected} label="Дата назначена" onChange={(value) => setDraft((current) => ({ ...current, thirdCertificationScheduled: value, thirdCertificationDate: value ? current.thirdCertificationDate : null }))} />
          <input
            type="date"
        ...
        <DatePanel title="9. Зум-подготовка к 3 аттестации" value={draft.thirdCertificationPreparationZoomDate} disabled={isRejected} onChange={(value) => setField('thirdCertificationPreparationZoomDate', value)} />
        <BooleanPanel title="10. Распределение рабочих часов" checked={draft.workingHoursAssigned} disabled={isRejected} label="Часы распределены" onChange={(value) => setField('workingHoursAssigned', value)} />
        <DatePanel title="11. Первая рабочая смена" value={draft.firstShiftDate} disabled={isRejected} onChange={(value) => setField('firstShiftDate', value)} />
      </div>
    </GlassCard>
  );
}
