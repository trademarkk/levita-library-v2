...
export async function readStateFromTables(supabase) {
  ...
  return {
    ...
    state: {
      ...
      expenses: expenses.map((expense) => ({ id: expense.id, date: expense.expense_date, amount: Number(expense.amount) || 0, account: expense.account, category: expense.category, studio: expense.studio, comment: expense.comment, createdAt: expense.created_at })),
